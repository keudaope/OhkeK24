...
namespace T18_Oppilaitokset
{
    public partial class OppilaitoksetFM : Form
    {
        ...
        private void OppilaitosCB_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            //MessageBox.Show("OID = " + viite);
            OppilaitosOsoiteLB.Text = oppilaitos.Rows[OppilaitosCB.SelectedIndex]["OKatuosoite"].ToString();
            OppilaitosPnroLB.Text = oppilaitos.Rows[OppilaitosCB.SelectedIndex]["OPostinumero"].ToString();
            OppilaitosPtpLB.Text = oppilaitos.Rows[OppilaitosCB.SelectedIndex]["OPostitoimipaikka"].ToString();
            OppilaitosPuhLB.Text = oppilaitos.Rows[OppilaitosCB.SelectedIndex]["OPuhelin"].ToString();

            
            //yhteys = vastuuHenkilot.Select("OID =" + viite).CopyToDataTable();
            VastuuHloCB.DataSource = vastuuHenkilot;
        ...
        {
            VastuutitteliLB.Text = yhteys.Rows[VastuuHloCB.SelectedIndex]["VTitteli"].ToString();
            OsastoLB.Text = yhteys.Rows[VastuuHloCB.SelectedIndex]["VSijainti"].ToString();
            SahkopostiLB.Text = yhteys.Rows[VastuuHloCB.SelectedIndex]["VSahkoposti"].ToString();
            VastuuhloPuhLB.Text = yhteys.Rows[VastuuHloCB.SelectedIndex]["VPuhelin"].ToString();
            VastuutitteliLB.Visible = true;
            OsastoLB.Visible = true;
            SahkopostiLB.Visible = true;
            VastuuhloPuhLB.Visible = true;
        }
    }
}